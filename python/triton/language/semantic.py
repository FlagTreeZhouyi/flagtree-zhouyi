...
def _load_block_pointer(ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, my_hints, builder):
...
def _load_legacy(ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, my_hints, builder):
    ...
    # Build IR
    if mask is None:
        ret = tl.tensor(builder.create_load(ptr.handle, cache, eviction, is_volatile, my_hints), dst_ty)
    else:
        ret = tl.tensor(
            builder.create_masked_load(ptr.handle, mask.handle, other.handle if other else None, cache, eviction,
                                       is_volatile, my_hints), dst_ty)
    if is_bool:
...
def load(ptr: tl.tensor, mask: Optional[tl.tensor], other: Optional[tl.tensor], boundary_check: Tuple,
         padding_option: str, cache_modifier: str, eviction_policy: str, is_volatile: bool, my_hints: str,
         builder: ir.builder) -> tl.tensor:
